...
# Added logs exclusion to avoid inflated counts from rotating log files
EXCLUDE_PATHS="-not -path '*/\.*' -not -path '*/__pycache__/*' -not -path '*/.pytest_cache/*' -not -path '*/.mypy_cache/*' -not -path '*/.tox/*' -not -path '*/venv/*' -not -path '*/env/*' -not -path '*/.env/*' -not -path '*/node_modules/*' -not -path '*/target/*' -not -path '*/bin/*' -not -path '*/obj/*' -not -path '*/.vs/*' -not -path '*/.idea/*' -not -path '*/.vscode/*' -not -path '*/coverage/*' -not -path '*/dist/*' -not -path '*/build/*' -not -path '*/logs/*' -not -path '*/public/*' -not -path '*/downloaded-game-assets/*'"
...
if [ $FULL -eq 1 ]; then
    ...
    eval "find \"$TARGET_DIR\" -type f $EXCLUDE_PATHS 2>/dev/null | grep -E \"\.(${CODE_EXTENSIONS})$|requirements.*\.txt$|Pipfile$|pyproject\.toml$|Cargo\.toml$|Cargo\.lock$|package\.json$|\.csproj$|\.sln$|Gemfile$|composer\.json$|Makefile$|Dockerfile$\" | sort" | while read -r file; do
        ...
        # For very large files (>1MB) capture first 1000 lines across 5 chunks for better context
        if [ "$file_size" -gt 1048576 ]; then
            echo "Chunking large file: $file ($file_size bytes)"
            echo "==============================================" >> "$CONTENT_REPORT"
            echo "FILE: $file (CHUNKED - Large: $file_size bytes)" >> "$CONTENT_REPORT"
            max_lines=1000
            chunk_size=200
            lines_read=0
            while [ $lines_read -lt $max_lines ]; do
                start=$((lines_read + 1))
                end=$((lines_read + chunk_size))
                echo "---- LINES $start-$end ----" >> "$CONTENT_REPORT"
                sed -n "${start},${end}p" "$file" >> "$CONTENT_REPORT" 2>/dev/null || break
                lines_read=$((lines_read + chunk_size))
            done
            echo "---- (TRUNCATED AFTER ${max_lines} LINES) ----" >> "$CONTENT_REPORT"
            echo "" >> "$CONTENT_REPORT"
            continue
        fi
        echo "==============================================" >> "$CONTENT_REPORT"
