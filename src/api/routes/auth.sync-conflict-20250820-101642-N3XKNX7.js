...
// Default seed users are only enabled in non-production unless ALLOW_DEFAULT_USERS=1
const seedUsers = [
...
const allowDefaults = process.env.ALLOW_DEFAULT_USERS === '1' || (process.env.NODE_ENV !== 'production');
const users = allowDefaults ? seedUsers : [];
if (!allowDefaults && seedUsers.length) {
  logger.warn('Default in-memory users disabled in production. Set ALLOW_DEFAULT_USERS=1 to re-enable (NOT RECOMMENDED).');
}
