...
export default function BaseDesigner() {
  ...
  return (
    <div className="space-y-6">
                ...
                {palette.map(p=>{
                  ...
                  return (
                    <button
                      ...
                      title={p}
                      draggable
                      onDragStart={e=>{ e.dataTransfer.setData('text/plain', p); e.dataTransfer.effectAllowed='copy'; }}
                      onClick={()=>setSelectedType(p)}
                      className={`relative group border rounded flex items-center justify-center h-12 text-[9px] font-medium tracking-tight select-none ${active?'border-indigo-500 bg-indigo-500/20':'border-gray-700 bg-gray-800/40 hover:border-gray-500'}`}>
                      {icon ? (
                        <img src={icon} alt="" className="max-w-[85%] max-h-[85%] object-contain pointer-events-none" />
                      ) : (
                        <span className="pointer-events-none">{p.slice(0,3)}</span>
                      )}
                      {/* Hover Preview */}
                    ...
                    {[...Array(GRID_SIZE)].map((__,x)=>{
                      ...
                      return (
                        <div key={x}
                          ...
                          onDrop={e=>{ e.preventDefault(); const t = e.dataTransfer.getData('text/plain'); if(t){ setSelectedType(t); onCellClick(x,y); } }}
                          title={`${x},${y}`}
      ...
      {/* Side panel placeholder for WarLivePanel */}
      <div className="hidden md:block md:w-64 lg:w-72 xl:w-80 p-4 border-l border-gray-700">
        <WarLivePanel clanTag={primaryClanTag} compact />
      </div>
    </div>
  )
}
