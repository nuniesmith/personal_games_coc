...
class TicketCommands(TicketClick, commands.Cog, name='Ticket Commands'):
    ...
    async def ticket_panel_post(
        ...
        embeds = [disnake.Embed.from_dict(data=e) for e in embed_data.get('embeds', [])]
        await ctx.channel.send(content=embed_data.get('content', '') or '', embeds=embeds, components=all_buttons)
    ...
    async def ticket_apply_rules(
        ...
        embed = disnake.Embed(
            description=f'**Copy the text above**\n'
    ...
    async def on_message(self, message: disnake.Message):
        if not message.author.bot and isinstance(message.channel, disnake.TextChannel) and message.channel.topic == 'Ticketing powered by Feast!':
