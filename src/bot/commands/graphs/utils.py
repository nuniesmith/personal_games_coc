...
from utility.clash.other import gen_season_start_end_as_iso, gen_season_start_end_as_timestamp
...
async def daily_graph(
    ...
    fig.update_layout(
        title=dict(
            text=f"{attribute.replace('_', ' ').title()} ({months} months) | Made w/ ❤️ by Feast",
