...
class War(commands.Cog):
    ...
    @war.sub_command(name='prep', description='Show war preparation stats (API aggregated)')
    async def war_prep(
        self,
        ctx: disnake.ApplicationCommandInteraction,
        clan: coc.Clan = commands.Param(converter=convert.clan, autocomplete=autocomplete.clan),
        include_heroes: bool = commands.Param(default=False, description='Sample heroes for averages'),
        hero_sample_size: int = commands.Param(default=30, description='Hero sample size (<=50)'),
    ):
        from utility.prep_stats import fetch_prep_stats, build_prep_embeds
        await ctx.response.defer()
        # Expect bot.config to have API_BASE like http://api:3001/api ; fallback local dev
        api_base = getattr(self.bot, 'api_base', None) or 'http://localhost:3001/api'
        try:
            data = await fetch_prep_stats(api_base, clan.tag, include_heroes=include_heroes, hero_sample_size=hero_sample_size)
            embeds = build_prep_embeds(data)
            await ctx.send(embeds=embeds[:10])
        except Exception as e:
            await ctx.send(f'Failed to fetch prep stats: {e}', ephemeral=True)

    @war.sub_command(name='assign', description='Generate war assignments preview')
    async def war_assign(
        self,
        ctx: disnake.ApplicationCommandInteraction,
        clan: coc.Clan = commands.Param(converter=convert.clan, autocomplete=autocomplete.clan),
        war_size: int = commands.Param(default=15, description='War size'),
        algorithm: str = commands.Param(choices=['strength','mirror','optimal'], default='strength'),
    ):
        await ctx.response.defer()
        try:
            ...
            async for p in self.bot.coc_client.get_players(tags):
                ...
                players.append({
                    ...
                    'th': p.town_hall,
                    'trophies': p.trophies,
                    'heroes': [{'name': h.name, 'level': h.level} for h in p.heroes]
                })
            from utility.assignment import generate_assignments
            assigns = generate_assignments(players, war_size=war_size, algorithm=algorithm)
            lines = [f"#{a['slot']:>2} TH{a['th']} {a['name']} (w:{a['weight']})" for a in assigns]
            desc = '\n'.join(lines)
            embed = disnake.Embed(title=f"Assignments {war_size}v{war_size} ({algorithm})", description=desc[:3900], colour=disnake.Colour.green())
            await ctx.send(embed=embed)
        except Exception as e:
            await ctx.send(f'Assignment error: {e}', ephemeral=True)
