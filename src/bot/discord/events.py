...
class DiscordEvents(commands.Cog):
    ...
    async def on_shard_ready(self, shard_id: int):
        ...
        await self.bot.change_presence(
            activity=disnake.CustomActivity(state='Use Code Feast 👀', name='Custom Status'),
            shard_id=shard.id,
        )
    ...
    async def on_guild_join(self, guild: disnake.Guild):
        ...
        embed = disnake.Embed(
            ...
            description=f'**Feast has joined the server:** `{server_name}`',
        ...
        msg = (
            '# Thanks for inviting **Feast**! 🎉\n\n'
            'Feast is designed to simplify clan and family management while providing powerful features like **legends tracking**, **autoboards**, and **in-depth stats**. '
            'It also includes tools like **role management**, **ticketing**, and **roster management** to make running your clan easier.\n\n'
            'To get started, browse the [documentation](https://docs.feast.xyz) & run `/help` to explore the available features.'
            ' For additional support, you can also query our docs with `/ask` or join the [support/community server](https://discord.gg/feast).\n\n'
            '**Note:** Feast is actively developed and improving constantly. If you encounter any issues or have feature suggestions, let me know! If you enjoy the bot, '
            'consider supporting the project by using Creator Code **Feast** in-game. Thank you for being part of this journey! - Destinea, Magic, & Obno ❤️'
        )
        ...
        buttons.append_item(disnake.ui.Button(label='Support Server', emoji='🔗', url='https://discord.gg/feast'))
        buttons.append_item(disnake.ui.Button(label='Documentation', emoji='🔗', url='https://docs.feast.xyz'))
    ...
    async def on_guild_remove(self, guild):
        ...
        embed = disnake.Embed(
            title='Left a Server 🛑',
            description=f'**Feast has been removed from the server:** `{server_name}`',
    ...
    async def on_slash_command_completion(self, ctx: disnake.ApplicationCommandInteraction):
        ...
        try:
            last_run = await self.bot.command_stats.find_one(
                filter={'$and': [{'user': ctx.author.id}, {'sent_support_msg': True}]}, sort=[('time', -1)]
            )
            ...
            if last_run is None or current_time - last_run.get('time', 0) >= run_time_check_seconds:
                ...
                buttons = disnake.ui.ActionRow(
                    disnake.ui.Button(label='Discord', style=disnake.ButtonStyle.url, url='https://discord.gg/3HVBqc44')
                )

                embed.set_thumbnail(url=self.bot.user.avatar.url)

                # Send the message
                await ctx.followup.send(embed=embed, components=[buttons], delete_after=60, ephemeral=True)
