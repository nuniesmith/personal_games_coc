...
_DISCORD_LINKS_WARN_SHOWN = False  # module-level guard


def _init_discord_links(config):
    """Initialize DiscordLinks client or return stub; log warning once."""
    global _DISCORD_LINKS_WARN_SHOWN
    lu = (getattr(config, 'link_api_username', '') or '').strip()
    lp = (getattr(config, 'link_api_password', '') or '').strip()
    if lu and lp:
        try:
            return asyncio.get_event_loop().run_until_complete(discordlinks.login(lu, lp))
        except Exception as e:  # fall through to stub
            if not _DISCORD_LINKS_WARN_SHOWN:
                print(f'[WARN] DiscordLinks login failed: {e}. Continuing without link features.')
                _DISCORD_LINKS_WARN_SHOWN = True
    if not _DISCORD_LINKS_WARN_SHOWN:
        if not (lu and lp):
            print('[WARN] DiscordLinks credentials not provided (LINK_API_USER / LINK_API_PASSWORD). Feature disabled.')
        _DISCORD_LINKS_WARN_SHOWN = True
    class _DummyLinkClient:
        async def get_linked_players(self, *args, **kwargs):
            return []
    return _DummyLinkClient()
...
class CustomClient(commands.AutoShardedBot):
    def __init__(
        ...
        **kwargs,
        ...
        self.excel_templates: collection_class = self.looper_db.feast.excel_templates
        self.giveaways: collection_class = self.looper_db.feast.giveaways
        self.tokens_db: collection_class = self.looper_db.feast.tokens
        self.lineups: collection_class = self.looper_db.feast.lineups
        self.bot_sync: collection_class = self.looper_db.feast.bot_sync
        self.army_share: collection_class = self.looper_db.feast.army_share

        # Discord Links (optional)
        self.link_client = _init_discord_links(self._config)
        self.bot_stats: collection_class = self.looper_db.feast.bot_stats
        ...
        self.autoboards: collection_class = self.looper_db.feast.autoboards
        ...
        self.number_emojis: collection_class = self.looper_db.feast.number_emojis
    ...
    async def white_list_check(self, ctx, command_name):
        ...
        if disnake.utils.get(member.roles, name='Feast Perms') is not None:
            return True
